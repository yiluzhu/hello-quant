...
1. Formula
    A = fi * S * exp((b - r) * T) * N(fi * x1) - 
        fi * X * exp(-r * T) * N(fi * x1 - fi * vol * sqrt(T))
    
    B = fi * S * exp((b - r) * T) * N(fi * x2) - 
        fi * X * exp(-r * T) * N(fi * x2 - fi * vol * sqrt(T))
...
from option import Option, OptionType, cdf
...
class BarrierOption(Option):
    ...
    def __init__(self, spot, strike, rate, expiry, vol, coc, rebate, bar):
        super(BarrierOption, self).__init__(spot, strike, rate, expiry, vol)
